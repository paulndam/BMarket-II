...
import { orderListLatest } from "../Actions/OrderLatestAction";

const OrderHistoryScreen = (props) => {
  const orderListReducer = useSelector((state) => state.orderListReducer);
  const { loading, error, Order } = orderListReducer;

  const orderLatestListReducer = useSelector(
    (state) => state.orderLatestListReducer
  );
  const {
    loading: loadingL,
    error: errorL,
  ...
  return (
    <div>
          ...
          <tbody>
            {Order.map((orders) => (
              <tr>
                ...
                <td>
                  {orders.isPaid ? (
                    <sapn className='D-yes' >Paid On {orders.paidAt.substring(0, 10)}</sapn>
                  ) : (
                ...
                <td>
                  {orders.isDelivered ? (
                    <span className="D-yes">Delivered On {orders.deliveredAt.substring(0, 10)}</span>
                  ) : (
      ...
      {/* latest product Order history table  */}
      ...
      {loadingL ? (
        ...
        <MessageBox variant="danger">{errorL}</MessageBox>
      ) : (
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            {OrderL.map((orders) => (
              <tr>
                ...
                <td>
                  {orders.isPaid ? (
                    <span className='D-yes'>Paid On {orders.paidAt.substring(0, 10)} </span>
                  )  : (
                    <span className="D-no">Not Paid</span>
                ...
                <td>
                  <button
                    ...
                    id="orderHBTN"
                    type="button"
                    onClick={() => props.history.push(`/orderlatest/${orders._id}`)}>
                    details
                  </button>
                </td>
              </tr>
