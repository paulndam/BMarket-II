...
import { createLatestOrder } from "../Actions/OrderLatestAction";
import { ORDER_LATEST_CREATE_RESET } from "../Constants/OrderLatestConstant";

const PlaceLatestOrderScreen = (props) => {
  ...
  if (!LatestCartReducer.paymentMethod) {
    props.history.push("/paymentlatestproduct");
  }

  const orderLatestCreateReducer = useSelector((state) => state.orderLatestCreateReducer);
  const { loading, success, error, Order } = orderLatestCreateReducer;

  const dispatch = useDispatch();

  const toPrice = (num) => Number(num.toFixed(2));
  LatestCartReducer.itemsPrice = toPrice(
    LatestCartReducer.latestCartItems.reduce((a, c) => a + c.qty * c.price, 0)
  );
  LatestCartReducer.shippingPrice =
    LatestCartReducer.itemsPrice > 100 ? toPrice(0) : toPrice(1.2);
  LatestCartReducer.taxPrice = toPrice(0.05 * LatestCartReducer.itemsPrice);
  LatestCartReducer.totalPrice =
    LatestCartReducer.itemsPrice +
    LatestCartReducer.shippingPrice +
    LatestCartReducer.taxPrice;

  const orderNow = () => {
    dispatch(
        createLatestOrder({
        ...LatestCartReducer,
        orderlatestItems: LatestCartReducer.latestCartItems,
      })
    );
  };
  ...
  useEffect(() => {
    if (success) {
      props.history.push(`/orderlatest/${Order._id}`);
      dispatch({ type: ORDER_LATEST_CREATE_RESET });
    }
      ...
      <div className="row">
        <div className="col-2">
          <div id="firstCard" className="order-card">
            ...
            <p>
              <strong>Name:</strong> {LatestCartReducer.shippingAddress.fullname}
              <br />
              <strong>Address: </strong> {LatestCartReducer.shippingAddress.address}
              <br />
              <strong>City: </strong>
              {LatestCartReducer.shippingAddress.city},<br />
              <strong>Zipcode: </strong>
              {LatestCartReducer.shippingAddress.zipcode}
              <br />
              <strong>Country: </strong>
              {LatestCartReducer.shippingAddress.country}
            </p>
          </div>
          ...
          <div id="secondCard" className="order-card">
            ...
            <p id="method">
              ...
              <br />
              {LatestCartReducer.paymentMethod}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="small-container cart-page">
        <table>
          <div>
            ...
            <tr>
              <div>
                {LatestCartReducer.latestCartItems.map((item) => (
                  <div>
                    <td>
                      <div class="cart-info" key={item.latestProducts}>
                        <Link to={`/product/${item.latestProducts}`} className="link">
        ...
        <div class="total-price">
          <table>
            <tr>
              <td>Item(s)</td>
              <td>${LatestCartReducer.itemsPrice.toFixed(2)}</td>
            </tr>
            <tr>
              <td>Shipping</td>
              <td>${LatestCartReducer.shippingPrice.toFixed(2)}</td>
            </tr>
            <tr>
              <td>Tax</td>
              <td>${LatestCartReducer.taxPrice.toFixed(2)}</td>
            </tr>
            <tr>
              <td>
                {" "}
              ...
              <td>
                {" "}
                <strong>${LatestCartReducer.totalPrice.toFixed(2)}</strong>{" "}
              </td>
            </tr>
            ...
            <button
              ...
              type="button"
              onClick={orderNow}
              disabled={LatestCartReducer.latestCartItems.length === 0}>
              Place Order
            </button>
            {loading && <Loading />}
...
export default PlaceLatestOrderScreen;
